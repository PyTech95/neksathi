...
## Update 2026-06 — Mobile menu + plans teaser + hero SOS demo (batch 36)
- Mobile hamburger (TopNav.jsx): .nav-hamburger button + right slide-in .mobile-drawer (CSS in index.css, shows <=992px, hides .nav-desktop). Drawer mirrors auth state (public: Solutions items + demo + Login + "Launch Nek Sathi"; user: Home/SOS/Family + Vehicle&QR + More groups + Settings/Admin/Logout; org/dealer branches). Closes on route change + item tap + overlay. testids mobile-menu-btn, mobile-drawer, m-*.
- Plans teaser (Landing.jsx): #plans section, 3 cards Free ₹0 / Guardian ₹99 (highlighted, Family favourite) / Super Family ₹499, 3 bullets + badge each, CTA -> /subscription. testids plan-teaser-{code}, plan-teaser-cta-{code}. NOTE: prices are teaser copy; real plans live on /subscription (Stripe price_cents 9900 & 49900 exist in backend).
- Hero SOS demo (Landing.jsx HeroDemo): auto-playing CSS/JS phone mockup looping 3 steps (tap SOS -> alert sent to 3 guardians + live location -> guardian on the way), progress dots + caption, "Watch full demo" opens a modal (hero-demo-modal) with enlarged sequence + "Set up my SOS" CTA. No video asset (pure CSS/JS). testids hero-demo, hero-demo-watch, hero-demo-modal, hero-demo-cta.
- Verified via screenshots (hero demo cycling, modal, plans cards) + forced-open drawer test (links present, m-login navigates + drawer closes).

## ROADMAP — big "family safety super-app" features requested (batch 36 backlog)
WEB-FEASIBLE (can build in-app next): Safety Check timers (auto-SOS if not confirmed), Fake Call "Exit Toolkit" (client-side), Safe-Word + Verify Status (extends check-in/nudge), No-Show/ETA alerts (extends safe zones + timers), Temporary Circles (time-boxed family sharing), Ambient Incident/Crime map (crowd-sourced), Guardian roles/load-balancing, Safety Score dashboard, Scam-call flag + playbooks content, DPDP audit logs + Evidence Export, Medical QR / lock-screen ICE card (QR already exists), Identity/breach alerts (premium).
NATIVE-MOBILE ONLY (needs Expo app; web can only prep APIs): Crash detection (accelerometer), Duress PIN + Ghost mode, Fake Power-Off shield, OS-level fake-call detection, SIM/lock-screen tamper hooks, USSD/missed-call SOS, wearable BLE triggers, OBD vehicle module, background local audio/video recording.

## Update 2026-06 — OTP login fix + Temporary Circles verified (batch 37, this fork)
- OTP LOGIN BUG (P0) FIXED & VERIFIED: WhatsApp OTP codes are self-generated + stored in db.otp_codes and verified LOCALLY (server.py otp_verify ~6460 branches on comms.whatsapp_otp_live()). MSG91 SMS-OTP path still uses comms.verify_otp. Confirmed live in this pod: whatsapp_otp_live()=True so request returns dev_code=null (code sent via WhatsApp template); verify against DB code returns a valid JWT + auto-creates the user.
- TEMPORARY CIRCLES (M509) COMPLETE & VERIFIED: backend routes /api/circles/temp (create/join/list/get/ping/leave/end) + TemporaryCircles.jsx (create with 1-24h expiry, join by 6-char code, live Leaflet map of members, 12s refresh, geolocation watch ping, owner "End circle" / member "Leave"). Auto-expires via _active_temp_circle expiry check.
- Self-verified end-to-end via curl: OTP request→(DB code)→verify→JWT; create circle→ping→get shows my location; 2nd user joins by code→pings→owner sees BOTH members with locations; list; owner-end. Frontend serves clean (landing screenshot OK). Test data cleaned up. NOT run through testing_agent (user opted for self-verify).
- REMAINING BACKLOG (unchanged): VirusTotal link/file scanner (needs user API key), Smart Commute 2.0, Digital Scam & Voice Verification, Granular Place Alerts & Routines, Safety Check Timers, Fake Call toolkit, Offline/USSD failsafes, native-mobile-only items.

## Update 2026-06 — Vehicle-scan call: telephony paused + in-app live voice call (batch 38, this fork)
- PAUSED the old masked telephony call on the public vehicle-scan page. The scan flow now shows only the "Alert owner" reasons + a clean "Owner alerted" confirmation. The Vobiz telephony/dialpad code is kept behind a single flag `CALL_ENABLED=false` in `PublicScan.jsx` (re-enable later). Removed the "connect a private call" copy from emergency/theft descriptions.
- ADDED a REAL in-app live voice call (self-hosted WebRTC) so an anonymous scanner can talk to the vehicle owner INSIDE the app — no phone numbers on either side.
  - Backend (`server.py`, new "In-app live voice call" section ~L6055): `webrtc_calls` collection + REST signaling (short-polling). Public/anon: `POST /public/qr/{qr_id}/call/start`, `POST /public/call/{id}/offer`, `POST /public/call/{id}/candidate`, `GET /public/call/{id}`, `POST /public/call/{id}/end`. Owner/auth: `GET /me/calls/incoming`, `GET /me/calls/{id}`, `POST /me/calls/{id}/accept|reject|candidate|end`. STUN only (Google/Twilio public), no paid TURN. Ring timeout 45s → status `missed` + WhatsApp/push alert to owner+family + `alerts` row (`_notify_missed_call`, `_expire_stale_calls`). Anonymous — scanner shares nothing; owner id/number never returned.
  - Frontend: `lib/rtc.js` (RTC_CONFIG + WebAudio ringtone); `components/InAppCall.jsx` (caller overlay: mic → offer → poll answer/ICE → connected/timer/mute/hangup); `components/IncomingCall.jsx` (global owner listener, mounted in TopNav for logged-in users, polls `/me/calls/incoming` every 3s, ringtone + Answer/Decline modal, runs answer flow). PublicScan: "Call owner now (live, private)" button on both the choose screen and the waiting screen; renders `<InAppCall/>`.
  - VERIFIED: full signaling handshake via curl (start→offer→candidate→owner incoming→accept→answer+ICE→caller sees accepted). Screenshots: scan choose shows call button; caller overlay shows "Ringing the owner…"; owner dashboard shows the "Incoming call" modal (Answer/Decline). NOTE: actual two-way P2P AUDIO needs two real browsers with mics (couldn't be auto-verified headless) and may fail on strict/symmetric NAT since there's no TURN server (accepted trade-off for self-hosted).

## Update 2026-06 — Live call verified end-to-end + TURN + selectable ringtone (batch 39, this fork)
- REAL two-peer call VERIFIED end-to-end: automated a caller page (scan) + owner page (dashboard, injected token) in one browser context — both reached WebRTC **"Connected"** with a live call timer. The in-app voice call media path works.
- TURN reliability: added free public TURN (OpenRelay by Metered, turn:openrelay.metered.ca 80/443 + 443?transport=tcp, user/cred "openrelayproject") to `RTC_CONFIG` in `lib/rtc.js` alongside Google STUN — connects on strict/symmetric NAT & mobile networks, no key/paid service.
- Ringtone + vibration + user-selectable in Settings:
...
## Update 2026-06 — Countdown ring + missed-call inbox (batch 43, this fork)
- Countdown Ring: `_incident_public` now also returns `expires_at`. New `CountdownRing` component in `PublicScan.jsx` shows a circular 15-min countdown (mm:ss, per-second, turns red in the last minute) on the waiting + owner-coming screens for windowed types (wrong_parking/vehicle_blocking). Soft WebAudio beeps at 60s and 10s remaining (`softBeep`, one-shot refs). Verified screenshot: ring shows 14:21 · "window left" + ALERT SENT chip.
- Missed-Call Inbox: new backend `GET /me/calls/recent?limit=` returns owner's recent non-ringing calls + `missed` count. Dashboard (`Dashboard.jsx`) fetches it in `load()` and renders a "Recent calls" section with a "N MISSED" badge and per-call rows (plate, relative time, colored status: Missed=red / Declined=amber / Answered=green). Verified via curl (aged call → missed) + screenshot (1 MISSED badge + missed row).

## Update 2026-06 — Missed-call link + Extend window + bilingual voice (batch 44, this fork)
...
## Session update — Native plugins + mobile-overlay fixes + fresh APK (2026-06)
- Native bridges added (native-only, inert web fallback): src/lib/native.js (Geolocation for SOS + Camera selfie), src/lib/push.js (@capacitor/push-notifications -> POST /api/register-push after login). Wired into Safety.jsx SOS and AuthContext. Android manifest perms + iOS Info.plist usage strings added. Push needs Firebase google-services.json to activate (build stays intact without it); documented in frontend/MOBILE_BUILD.md.
- Bug fixes (all testing_agent-verified): SOS double-fire (side-effect out of setCount + firedRef one-shot); FamilyAlarm overlay stealing panic-button clicks (wrapper pointer-events:none, cap 3 + '+N more' pill); CRITICAL mobile blockers — global overlays were inside .nav-desktop (display:none <992px), now portaled to document.body; banners suppressed on /safety (+ /family), capped to 1 on <=640px; overlay portal hidden (CSS display:none, no unmount) while mobile drawer open; drawer z-index raised to 4501/4500.
- Testing: iter7 backend 42/42; iter9 & iter10 frontend 100% (mobile 390x844 + desktop 1920). "App ready for mobile build", no open frontend issues.
- Rebuilt fresh debug APK (11.8MB, com.neksathi.app, includes FINE/COARSE_LOCATION + CAMERA + push): frontend/android/app/build/outputs/apk/debug/app-debug.apk, published at /downloads/neksathi.apk. (Built in-container via JDK17 + Android SDK34 + qemu-wrapped x86_64 aapt2 on the arm64 host; /opt is ephemeral so CI workflow is the reproducible path.)

## Session update — In-app QR scanner + active-SOS tracking (2026-06)
- In-app QR scanner: /scanner (Protected), nav 'Scan' (data-testid nav-scanner desktop, m-scanner mobile). Uses html5-qrcode (web + Capacitor webview); decodes Nek Sathi /scan//t//c/ URLs and routes in-app. #qr-reader preview height capped for mobile.
- Active-SOS location tracking: src/lib/native.js startSosTracking/stopSosTracking; Safety.jsx fire() starts posting /api/me/location on an 8s interval after SOS, stops on 'I'm safe' ack + on unmount. Android ACCESS_BACKGROUND_LOCATION/FOREGROUND_SERVICE_LOCATION + iOS UIBackgroundModes:location added. NOTE: true killed-app background needs a background-geolocation plugin (documented in MOBILE_BUILD.md).
- Testing iteration_11: 100% of requested scenarios (scanner reaches scanning state w/ fake camera, nav entries work, SOS single-fire holds, /me/location posts during active SOS). No functional defects. Known-optional: SOS banner stack overlaps page headers; ~10s SOS send shows only a spinner. Code-review note: LiveShare loop + SOS tracker both post /me/location (dup when both active); tracker stops on navigating away from /safety.
- Refreshed debug APK (18.4MB) at /downloads/neksathi.apk with scanner + tracking + all fixes.

## Session update — VPS: vehicle scan alert WhatsApp/SMS fix (2026-09)
- Root cause of "ring & alert not going" on VPS: (a) public_qr_alert (vehicle scan) only sent a PUSH + stored contact numbers — it never fired WhatsApp/SMS (unlike tag flow); (b) WhatsApp alert needs MSG91_WHATSAPP_TEMPLATE (different from the OTP template) and SMS needs MSG91_SMS_FLOW_ID — not set on VPS -> mock; (c) masked-call "ring" is in-app WebRTC (owner app must be open) or Vobiz phone-call which needs VOBIZ_AUTH_ID/TOKEN/MASKING_DID + a correct public PUBLIC_APP_URL for webhooks.
- FIX (code): added WhatsApp + SMS fan-out (notify_whatsapp + send_sms fallback, de-duped via comms.e164) to owner + opted-in vehicle contacts in public_qr_alert. Verified testing_agent iter13 13/13 (mock send logged + audited in db.notifications kind=scan_alert); no regressions.
- VPS ACTION REQUIRED (env, then restart backend): MSG91_AUTHKEY, MSG91_WHATSAPP_NUMBER, MSG91_WHATSAPP_TEMPLATE (approved utility alert template), MSG91_SMS_FLOW_ID (+MSG91_SMS_SENDER); for masked calls VOBIZ_AUTH_ID, VOBIZ_AUTH_TOKEN, VOBIZ_MASKING_DID; PUBLIC_APP_URL=the VPS public https URL; ensure egress to control.msg91.com + Vobiz and public inbound HTTPS for /api/vobiz/* webhooks.
